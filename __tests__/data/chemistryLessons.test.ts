...
import { chemistryTopics, getAllChemistryLessons } from '@/lib/data/chemistryLessons';

describe('chemistryLessons data integrity', () => {
  const allTopicKeys = Object.keys(chemistryTopics) as (keyof typeof chemistryTopics)[];
  ...
  it('has at least one lesson per topic', () => {
    for (const key of allTopicKeys) {
      expect(chemistryTopics[key].length).toBeGreaterThan(0);
    }
  });

  it('getAllChemistryLessons returns all lessons with their category', () => {
    const all = getAllChemistryLessons();
    const totalFromTopics = Object.values(chemistryTopics).flat().length;
  ...
  for (const topicKey of expectedTopics) {
    describe(`topic: ${topicKey}`, () => {
      const lessons = chemistryTopics[topicKey as keyof typeof chemistryTopics];
  ...
  it('has no duplicate lesson titles', () => {
    const allLessons = Object.values(chemistryTopics).flat();
