...
import { HistoryLesson, TudorTopicCategory, QuizQuestion } from '@/types';
import {
  tudorTopics,
  getAllTudorLessons,
  tudorCategoryNames,
...
export default function TudorInteractive() {
  ...
  const generateRandomLesson = (completed: string[]) => {
    const allLessons = getAllTudorLessons();
  ...
  const markComplete = () => {
    if (currentLesson && typeof window !== 'undefined') {
      ...
      localStorage.setItem('completedTudorLessons', JSON.stringify(newCompleted));
    }
  };
  ...
  const startQuiz = () => {
    if (currentLesson) {
      const vocabPool = collectVocabPool(tudorTopics as unknown as Record<string, HistoryLesson[]>);
  ...
  const totalLessons = getAllTudorLessons().length;
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-8">
      <Link
        ...
        className="inline-flex items-center gap-2 bg-white text-purple-600 border-2 border-purple-600 px-5 py-3 rounded-xl font-semibold hover:bg-purple-600 hover:text-white transition-all hover:-translate-y-1 mb-6"
      >
        ← Back to History Quest
      </Link>

      {/* Header Section */}
      <div className="bg-white p-8 rounded-3xl shadow-xl mb-6 border-[3px] border-purple-500">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-gradient-to-br from-amber-100 to-amber-200 p-5 rounded-2xl">
            ...
            <div className="text-xl font-semibold text-gray-800">
              {tudorCategoryNames[currentCategory]}
            </div>
          </div>
        </div>
      ...
      {viewMode === 'browse' && (
        <div className="space-y-6">
          {(Object.keys(tudorTopics) as TudorTopicCategory[]).map((category) => (
            <div key={category} className="bg-white p-6 rounded-3xl shadow-lg">
              <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-3">
                <span className="text-3xl">{tudorCategoryIcons[category]}</span>
                {tudorCategoryNames[category]}
              </h2>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {tudorTopics[category].map((lesson, idx) => {
      ...
      {viewMode === 'lesson' && currentLesson && (
        <div className="space-y-6">
          {/* Lesson Header */}
          <div className="bg-gradient-to-r from-purple-600 to-indigo-700 text-white p-8 rounded-3xl shadow-xl">
            <div className="flex items-center gap-2 text-purple-200 mb-2">
              {tudorCategoryIcons[currentCategory]} {tudorCategoryNames[currentCategory]}
            </div>
            <h2 className="text-3xl font-bold mb-2">{currentLesson.title}</h2>
            <p className="text-purple-100 text-lg">{currentLesson.era}</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              onClick={handleNewLesson}
              className="flex-1 bg-purple-600 text-white px-8 py-4 rounded-2xl font-bold text-lg hover:bg-purple-700 hover:-translate-y-1 hover:shadow-xl transition-all flex items-center justify-center gap-3"
            >
              <span>🔄</span>
