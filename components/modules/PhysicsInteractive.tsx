...
import { PhysicsLesson, PhysicsTopicCategory } from '@/types';
import {
  physicsTopics,
  getAllPhysicsLessons,
  physicsCategoryNames,
  physicsCategoryIcons,
  physicsCategoryColors,
} from '@/lib/data/physicsLessons';
...
function ForceDiagram() {
  return (
    <svg
      viewBox="0 0 240 240"
      className="w-40 h-40"
      aria-label="Force diagram showing an object with force arrows"
    >
      {/* Object box */}
      <rect x="90" y="90" width="60" height="60" rx="8" fill="#3b82f6" opacity="0.85" />
      <text x="120" y="125" textAnchor="middle" fill="white" fontSize="11" fontWeight="bold">
        object
      </text>

      {/* Force arrows */}
      {/* Up arrow — normal / lift */}
      <line x1="120" y1="88" x2="120" y2="45" stroke="#22d3ee" strokeWidth="3" />
      <polygon points="120,38 114,50 126,50" fill="#22d3ee" />
      <text x="128" y="60" fill="#22d3ee" fontSize="10" fontWeight="bold">
        N
      </text>

      {/* Down arrow — weight */}
      <line x1="120" y1="152" x2="120" y2="195" stroke="#f97316" strokeWidth="3" />
      <polygon points="120,202 114,190 126,190" fill="#f97316" />
      <text x="128" y="195" fill="#f97316" fontSize="10" fontWeight="bold">
        W
      </text>

      {/* Right arrow — applied force */}
      <line x1="152" y1="120" x2="195" y2="120" stroke="#22c55e" strokeWidth="3" />
      <polygon points="202,120 190,114 190,126" fill="#22c55e" />
      <text x="186" y="112" fill="#22c55e" fontSize="10" fontWeight="bold">
        F
      </text>

      {/* Left arrow — friction */}
      <line x1="88" y1="120" x2="45" y2="120" stroke="#f43f5e" strokeWidth="3" />
      <polygon points="38,120 50,114 50,126" fill="#f43f5e" />
      <text x="42" y="112" fill="#f43f5e" fontSize="10" fontWeight="bold">
        f
      </text>
    </svg>
  );
  ...
  return (
    <button
      ...
      className={`relative w-full min-h-[90px] rounded-2xl p-4 text-left transition-all duration-300 shadow-md border-2 cursor-pointer
        ${flipped ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-blue-200 text-gray-800 hover:border-blue-400'}`}
...
// ─── Quiz Panel ───────────────────────────────────────────────────────────────
function QuizPanel({ lesson }: { lesson: PhysicsLesson }) {
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-cyan-50 p-6 rounded-3xl shadow-lg border-2 border-blue-200">
      ...
      <div className="mt-6 flex gap-3">
        {!submitted ? (
          <button
            ...
            className="flex-1 bg-blue-600 text-white px-6 py-3 rounded-2xl font-bold hover:bg-blue-700 hover:-translate-y-1 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:translate-y-0"
          >
            Submit Answers
          </button>
        ) : (
          <button
            onClick={reset}
            className="flex-1 bg-cyan-600 text-white px-6 py-3 rounded-2xl font-bold hover:bg-cyan-700 hover:-translate-y-1 transition-all"
          >
            🔄 Try Again
          </button>
        )}
...
// ─── Experiment Steps ─────────────────────────────────────────────────────────
function ExperimentSteps({ experiment }: { experiment: PhysicsLesson['experiments'][number] }) {
...
// ─── Practice Questions ───────────────────────────────────────────────────────
function PracticeQuestions({ questions }: { questions: PhysicsLesson['practiceQuestions'] }) {
...
function FunFactTicker({ facts }: { facts: string[] }) {
  ...
  return (
    <div className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-2xl px-5 py-4 flex items-center gap-3 shadow-lg">
...
function CategoryPicker({
  ...
  selected: PhysicsTopicCategory | 'all';
  onSelect: (c: PhysicsTopicCategory | 'all') => void;
}) {
    ...
    <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
      <button
        ...
        className={`flex-shrink-0 px-4 py-2 rounded-xl font-semibold text-sm transition-all border-2
          ${selected === 'all' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-200 text-gray-700 hover:border-blue-400'}`}
      >
        All Topics
      </button>
      {categories.map((cat) => {
        const colors = physicsCategoryColors[cat];
        return (
          <button
            key={cat}
            onClick={() => onSelect(cat)}
            className={`flex-shrink-0 px-4 py-2 rounded-xl font-semibold text-sm transition-all border-2
              ${selected === cat ? `${colors.badge} text-white border-transparent` : 'bg-white border-gray-200 text-gray-700 hover:border-blue-400'}`}
          >
            {physicsCategoryIcons[cat]} {physicsCategoryNames[cat]}
          </button>
        );
      })}
    </div>
...
function LessonBrowser({
  ...
  selectedCategory: PhysicsTopicCategory | 'all';
  completedLessons: string[];
  onSelect: (lesson: PhysicsLesson) => void;
}) {
  const categories = Object.keys(physicsTopics) as PhysicsTopicCategory[];
        ...
        return (
          ...
          >
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {physicsTopics[cat].map((lesson) => {
                const done = completedLessons.includes(lesson.title);
                return (
                  <button
                    key={lesson.title}
                    onClick={() => onSelect(lesson)}
                    className="text-left bg-white rounded-2xl p-4 shadow-sm border-2 border-transparent hover:border-blue-400 hover:-translate-y-1 transition-all"
                  >
...
export default function PhysicsInteractive() {
  const readCompleted = (): string[] => {
    ...
    const saved = localStorage.getItem('completedPhysicsLessons');
    if (!saved) return [];
    try {
  ...
  const [selectedCategory, setSelectedCategory] = useState<PhysicsTopicCategory | 'all'>('all');
  const [expandedSections, setExpandedSections] = useState<Set<string>>(
    new Set(['objectives', 'concepts']),
  );

  const allLessons = getAllPhysicsLessons();
  ...
  const markComplete = () => {
    ...
    localStorage.setItem('completedPhysicsLessons', JSON.stringify(updated));
  };
    ...
    return (
      <div className="max-w-4xl mx-auto px-5 py-8">
        <div className="bg-white p-10 rounded-3xl shadow-xl text-center">
          ...
          <p className="text-xl text-gray-600">Charging up the circuits…</p>
        </div>
      </div>
    );
  }
  ...
  const categoryKey = currentLesson
    ? (Object.keys(physicsTopics).find((k) =>
        physicsTopics[k as PhysicsTopicCategory].some((l) => l.title === currentLesson.title),
      ) as PhysicsTopicCategory | undefined)
    : undefined;
  const catColors = categoryKey ? physicsCategoryColors[categoryKey] : null;
  ...
  return (
    <div className="max-w-4xl mx-auto px-5 py-8 space-y-6">
      ...
      <Link
        href="/"
        className="inline-flex items-center gap-2 bg-white text-blue-600 border-2 border-blue-600 px-5 py-3 rounded-xl font-semibold hover:bg-blue-600 hover:text-white transition-all hover:-translate-y-1"
      >
        ← Back to Cronx Academy
      </Link>

      {/* Header */}
      <div className="bg-white p-8 rounded-3xl shadow-xl border-[3px] border-blue-500">
        <div className="flex items-center gap-4 mb-6">
          <div className="text-5xl">⚡</div>
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold text-blue-600">Physics Explorer</h1>
            <p className="text-gray-600 mt-1">
              Forces, energy, waves, light, and space — for Thalia
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
          <div className="bg-gradient-to-br from-blue-100 to-blue-200 p-4 rounded-2xl">
            <div className="text-xs font-semibold text-blue-900 mb-1">📅 Today</div>
          ...
          <div className="col-span-2 sm:col-span-1 bg-gradient-to-br from-cyan-100 to-sky-200 p-4 rounded-2xl">
            <div className="text-xs font-semibold text-cyan-900 mb-1">📚 Topics</div>
            <div className="text-sm font-bold text-gray-800">5 physics topics</div>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <button
            ...
            className={`flex-1 py-3 rounded-2xl font-bold transition-all border-2 text-sm
              ${viewMode === 'lesson' ? 'bg-blue-600 text-white border-blue-600' : 'bg-gray-100 text-gray-700 border-gray-200 hover:bg-gray-200'}`}
          >
            🎲 Random Lesson
          </button>
          <button
            ...
            className={`flex-1 py-3 rounded-2xl font-bold transition-all border-2 text-sm
              ${viewMode === 'browse' ? 'bg-blue-600 text-white border-blue-600' : 'bg-gray-100 text-gray-700 border-gray-200 hover:bg-gray-200'}`}
          >
            🔍 Browse Topics
          </button>
        </div>
      ...
      {viewMode === 'lesson' && currentLesson && (
        <>
          ...
          {/* Lesson header */}
          <div
            className={`bg-gradient-to-r from-blue-600 to-cyan-700 text-white p-8 rounded-3xl shadow-xl relative overflow-hidden`}
          >
            {/* Decorative force diagram */}
            ...
            <div className="relative z-10">
              <div className="flex flex-wrap gap-2 mb-3">
                {catColors && (
                  ...
                  >
                    {categoryKey && physicsCategoryIcons[categoryKey]} {currentLesson.category}
                  </span>
                )}
          ...
          {/* Experiments */}
          <SectionToggle id="experiments" label="Investigations & Experiments" icon="🔬">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              ...
              className="flex-1 bg-blue-600 text-white px-8 py-4 rounded-2xl font-bold text-lg hover:bg-blue-700 hover:-translate-y-1 hover:shadow-xl transition-all flex items-center justify-center gap-3"
            >
              🔄 New Lesson
            </button>
          </div>
      ...
      <div className="bg-white p-8 rounded-3xl shadow-xl">
        ...
        <ul className="space-y-3">
          {[
            'Start with forces and motion — they connect directly to everyday life (cycling, throwing, falling) and build confidence before abstract topics like waves.',
            'The flip cards work brilliantly for physics equations: cover the equation and ask Thalia to recall it. Repetition builds fluency.',
            'Encourage Thalia to draw force diagrams for everyday situations — a ball being kicked, a book on a table — before attempting calculations.',
            'The experiment checklists help structure practical work; always review the safety note together first and discuss what variables to keep constant.',
            'Connect physics to Caribbean contexts where natural — hurricane dynamics, sailing, coral reef acoustics, space observation by indigenous peoples.',
            'For maths-heavy topics (Ohm\'s Law, wave speed), try the foundation lessons first then revisit standard level once the concept is secure.',
            'The Space topic is great for sparking curiosity — connect to the discovery of pulsars by Jocelyn Bell Burnell (Jamaican-British heritage) and Taíno astronomical knowledge.',
          ].map((tip, i) => (
            <li key={i} className="flex items-start gap-3 text-gray-700">
