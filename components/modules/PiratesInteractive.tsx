...
import { piratesTopics, getAllPiratesLessons, piratesCategoryNames, piratesCategoryIcons } from '@/lib/data/piratesLessons';
...
export default function PiratesInteractive() {
  ...
  const generateRandomLesson = (completed: string[]) => {
    const allLessons = getAllPiratesLessons();
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('completedPiratesLessons');
  ...
  const markComplete = () => {
    if (currentLesson && typeof window !== 'undefined') {
      ...
      localStorage.setItem('completedPiratesLessons', JSON.stringify(newCompleted));
    }
  };
    ...
    return (
      <div className="max-w-7xl mx-auto px-5 py-8">
        <div className="bg-white p-10 rounded-3xl shadow-xl text-center">
          ...
          <p className="text-xl text-gray-600">Loading your history lesson...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-8">
      <Link
        ...
        className="inline-flex items-center gap-2 bg-white text-orange-600 border-2 border-orange-600 px-5 py-3 rounded-xl font-semibold hover:bg-orange-600 hover:text-white transition-all hover:-translate-y-1 mb-6"
      >
        ← Back to History Quest
      </Link>

      {/* Header Section */}
      <div className="bg-white p-8 rounded-3xl shadow-xl mb-6 border-[3px] border-orange-500">
        <div className="flex items-center gap-4 mb-6">
          <div className="text-5xl">🏴‍☠️</div>
          <div>
            <h1 className="text-4xl font-bold text-orange-600">Pirates of the Caribbean</h1>
            <p className="text-gray-600">The golden age of piracy in Caribbean waters</p>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-gradient-to-br from-amber-100 to-amber-200 p-5 rounded-2xl">
            ...
            <div className="text-xl font-semibold text-gray-800">
              {piratesCategoryNames[currentCategory]}
            </div>
          </div>
        </div>
      ...
      {viewMode === 'browse' && (
        <div className="space-y-6">
          {(Object.keys(piratesTopics) as PiratesTopicCategory[]).map((category) => (
            <div key={category} className="bg-white p-6 rounded-3xl shadow-lg">
              <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-3">
                <span className="text-3xl">{piratesCategoryIcons[category]}</span>
                {piratesCategoryNames[category]}
              </h2>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {piratesTopics[category].map((lesson, idx) => {
      ...
      {viewMode === 'lesson' && currentLesson && (
        <div className="space-y-6">
          {/* Lesson Header */}
          <div className="bg-gradient-to-r from-orange-600 to-red-700 text-white p-8 rounded-3xl shadow-xl">
            <div className="flex items-center gap-2 text-orange-200 mb-2">
              {piratesCategoryIcons[currentCategory]} {piratesCategoryNames[currentCategory]}
            </div>
            <h2 className="text-3xl font-bold mb-2">{currentLesson.title}</h2>
            <p className="text-orange-100 text-lg">{currentLesson.era}</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-orange-50 to-orange-100 p-6 rounded-3xl shadow-lg border-l-[5px] border-orange-500">
            <button
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              onClick={handleNewLesson}
              className="flex-1 bg-orange-600 text-white px-8 py-4 rounded-2xl font-bold text-lg hover:bg-orange-700 hover:-translate-y-1 hover:shadow-xl transition-all flex items-center justify-center gap-3"
            >
              <span>🔄</span>
