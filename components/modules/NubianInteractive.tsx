...
import { HistoryLesson, NubianTopicCategory, QuizQuestion } from '@/types';
import {
  nubianTopics,
  getAllNubianLessons,
  nubianCategoryNames,
...
export default function NubianInteractive() {
  ...
  const [currentCategory, setCurrentCategory] = useState<NubianTopicCategory>('kingdomOfKush');
  const [completedLessons, setCompletedLessons] = useState<string[]>(() => {
    if (typeof window === 'undefined') return [];
    const saved = localStorage.getItem('completedNubianLessons');
  ...
  const generateRandomLesson = (completed: string[]) => {
    const allLessons = getAllNubianLessons();
  ...
  const markComplete = () => {
    if (currentLesson && typeof window !== 'undefined') {
      ...
      localStorage.setItem('completedNubianLessons', JSON.stringify(newCompleted));
    }
  };
  ...
  const selectLesson = (lesson: HistoryLesson, category: NubianTopicCategory) => {
    setCurrentLesson(lesson);
    setCurrentCategory(category);
    setViewMode('lesson');
  };
  ...
  const startQuiz = () => {
    if (currentLesson) {
      const vocabPool = collectVocabPool(
        nubianTopics as unknown as Record<string, HistoryLesson[]>,
      );
  ...
  const totalLessons = getAllNubianLessons().length;
    ...
    <div className="max-w-7xl mx-auto px-5 py-8">
      ...
      <div className="bg-white p-8 rounded-3xl shadow-xl mb-6 border-[3px] border-amber-500">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-4xl font-bold text-amber-600">Nubian Warrior Queens</h1>
            <p className="text-gray-600">Power, strategy, and legacy in the Kingdom of Kush</p>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-gradient-to-br from-indigo-100 to-indigo-200 p-5 rounded-2xl">
            ...
            <div className="text-xl font-semibold text-gray-800">
              {nubianCategoryNames[currentCategory]}
            </div>
          </div>
        </div>
      ...
      {viewMode === 'browse' && (
        <div className="space-y-6">
          {(Object.keys(nubianTopics) as NubianTopicCategory[]).map((category) => (
            <div key={category} className="bg-white p-6 rounded-3xl shadow-lg">
              <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-3">
                <span className="text-3xl">{nubianCategoryIcons[category]}</span>
                {nubianCategoryNames[category]}
              </h2>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {nubianTopics[category].map((lesson, idx) => {
      ...
      {viewMode === 'lesson' && currentLesson && (
        <div className="space-y-6">
          {/* Lesson Header */}
          <div className="bg-gradient-to-r from-amber-600 to-orange-700 text-white p-8 rounded-3xl shadow-xl">
            <div className="flex items-center gap-2 text-blue-200 mb-2">
              {nubianCategoryIcons[currentCategory]} {nubianCategoryNames[currentCategory]}
            </div>
            <h2 className="text-3xl font-bold mb-2">{currentLesson.title}</h2>
            <p className="text-blue-100 text-lg">{currentLesson.era}</p>
          </div>
          ...
          {/* Key Dates */}
          <div className="bg-gradient-to-br from-red-50 to-red-100 p-6 rounded-3xl shadow-lg border-l-[5px] border-red-500">
          ...
          <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 p-6 rounded-3xl shadow-lg border-l-[5px] border-indigo-500">
            <button
              ...
              className="w-full flex items-center justify-between text-xl font-bold text-indigo-900 mb-4"
            >
          ...
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-6 rounded-3xl shadow-lg border-l-[5px] border-purple-500">
            ...
            <div className="flex flex-wrap gap-2">
              {currentLesson.keyFigures.map((figure, idx) => (
                <span
                  key={idx}
                  className="bg-white px-4 py-2 rounded-full border border-purple-300 text-gray-700"
                >
                  {figure}
                </span>
              ))}
