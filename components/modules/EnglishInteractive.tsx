...
export default function EnglishInteractive() {
  ...
  const [currentCategory, setCurrentCategory] = useState<EnglishTopicCategory>('caribbeanAuthors');
  ...
  const markComplete = () => {
    if (currentLesson && typeof window !== 'undefined') {
      ...
      localStorage.setItem('completedEnglishLessons', JSON.stringify(newCompleted));
    }
  };
  ...
  const selectLesson = (lesson: HistoryLesson, category: EnglishTopicCategory) => {
    setCurrentLesson(lesson);
    setCurrentCategory(category);
    setViewMode('lesson');
  };
  ...
  const startQuiz = () => {
    if (currentLesson) {
      const vocabPool = collectVocabPool(englishTopics as unknown as Record<string, HistoryLesson[]>);
    ...
    return (
      <div className="max-w-7xl mx-auto px-5 py-8">
        <div className="bg-white p-10 rounded-3xl shadow-xl text-center">
          ...
          <p className="text-xl text-gray-600">Loading your English lesson...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-8">
      <Link
        href="/"
        className="inline-flex items-center gap-2 bg-white text-green-600 border-2 border-green-600 px-5 py-3 rounded-xl font-semibold hover:bg-green-600 hover:text-white transition-all hover:-translate-y-1 mb-6"
      >
        ← Back to Cronx Academy
      </Link>

      {/* Header Section */}
      <div className="bg-white p-8 rounded-3xl shadow-xl mb-6 border-[3px] border-green-500">
        <div className="flex items-center gap-4 mb-6">
          <div className="text-5xl">📖</div>
          <div>
            <h1 className="text-4xl font-bold text-green-600">English & Literature</h1>
            <p className="text-gray-600">
              Caribbean authors, literary analysis, creative writing, and poetry
            </p>
          </div>
        </div>

        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <div className="bg-gradient-to-br from-green-100 to-green-200 p-5 rounded-2xl">
          ...
          <div className="bg-gradient-to-br from-teal-100 to-teal-200 p-5 rounded-2xl">
            <div className="flex items-center gap-2 font-semibold text-teal-900 mb-2">
              📖 Current Topic
            </div>
            <div className="text-xl font-semibold text-gray-800">
              {englishCategoryNames[currentCategory]}
            </div>
          </div>
        </div>
      ...
      {viewMode === 'browse' && (
        <div className="space-y-6">
          {(Object.keys(englishTopics) as EnglishTopicCategory[]).map((category) => (
            <div key={category} className="bg-white p-6 rounded-3xl shadow-lg">
              <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-3">
                <span className="text-3xl">{englishCategoryIcons[category]}</span>
                {englishCategoryNames[category]}
              </h2>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {englishTopics[category].map((lesson, idx) => {
      ...
      {viewMode === 'lesson' && currentLesson && (
        <div className="space-y-6">
          {/* Lesson Header */}
          <div className="bg-gradient-to-r from-green-600 to-emerald-700 text-white p-8 rounded-3xl shadow-xl">
            <div className="flex items-center gap-2 text-green-200 mb-2">
              {englishCategoryIcons[currentCategory]} {englishCategoryNames[currentCategory]}
            </div>
            <h2 className="text-3xl font-bold mb-2">{currentLesson.title}</h2>
            <p className="text-green-100 text-lg">{currentLesson.era}</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-green-50 to-green-100 p-6 rounded-3xl shadow-lg border-l-[5px] border-green-500">
            <button
          ...
          <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 p-6 rounded-3xl shadow-lg border-l-[5px] border-yellow-600">
            ...
            >
              <span className="flex items-center gap-2">📜 Key Texts & Quotations</span>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              onClick={handleNewLesson}
              className="flex-1 bg-green-600 text-white px-8 py-4 rounded-2xl font-bold text-lg hover:bg-green-700 hover:-translate-y-1 hover:shadow-xl transition-all flex items-center justify-center gap-3"
            >
              <span>🔄</span>
