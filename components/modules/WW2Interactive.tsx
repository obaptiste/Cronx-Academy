...
import { HistoryLesson, WW2TopicCategory, QuizQuestion } from '@/types';
import {
  ww2Topics,
  getAllWW2Lessons,
  ww2CategoryNames,
...
export default function WW2Interactive() {
  ...
  const generateRandomLesson = (completed: string[]) => {
    const allLessons = getAllWW2Lessons();
  ...
  const markComplete = () => {
    if (currentLesson && typeof window !== 'undefined') {
      ...
      localStorage.setItem('completedWW2Lessons', JSON.stringify(newCompleted));
    }
  };
  ...
  const startQuiz = () => {
    if (currentLesson) {
      const vocabPool = collectVocabPool(ww2Topics as unknown as Record<string, HistoryLesson[]>);
  ...
  const totalLessons = getAllWW2Lessons().length;
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-8">
      <Link
        ...
        className="inline-flex items-center gap-2 bg-white text-pink-600 border-2 border-pink-600 px-5 py-3 rounded-xl font-semibold hover:bg-pink-600 hover:text-white transition-all hover:-translate-y-1 mb-6"
      >
        ← Back to History Quest
      </Link>

      {/* Header Section */}
      <div className="bg-white p-8 rounded-3xl shadow-xl mb-6 border-[3px] border-pink-500">
        <div className="flex items-center gap-4 mb-6">
          <div className="text-5xl">🌍</div>
          <div>
            <h1 className="text-4xl font-bold text-pink-600">World War II</h1>
            <p className="text-gray-600">An interactive journey through 1939-1945</p>
          </div>
        </div>

        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <div className="bg-gradient-to-br from-pink-100 to-pink-200 p-5 rounded-2xl">
          ...
          <div className="bg-gradient-to-br from-amber-100 to-amber-200 p-5 rounded-2xl">
            ...
            <div className="text-xl font-semibold text-gray-800">
              {ww2CategoryNames[currentCategory]}
            </div>
          </div>
        </div>
      ...
      {viewMode === 'browse' && (
        <div className="space-y-6">
          {(Object.keys(ww2Topics) as WW2TopicCategory[]).map((category) => (
            <div key={category} className="bg-white p-6 rounded-3xl shadow-lg">
              <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-3">
                <span className="text-3xl">{ww2CategoryIcons[category]}</span>
                {ww2CategoryNames[category]}
              </h2>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {ww2Topics[category].map((lesson, idx) => {
                    ...
                    <button
                      ...
                      className={`text-left p-4 rounded-xl border-2 transition-all hover:-translate-y-1 hover:shadow-lg ${
                        isCompleted
                          ...
                          : 'border-gray-200 bg-gray-50 hover:border-pink-400'
                      }`}
                    >
      ...
      {viewMode === 'lesson' && currentLesson && (
        <div className="space-y-6">
          {/* Lesson Header */}
          <div className="bg-gradient-to-r from-pink-600 to-red-700 text-white p-8 rounded-3xl shadow-xl">
            <div className="flex items-center gap-2 text-pink-200 mb-2">
              {ww2CategoryIcons[currentCategory]} {ww2CategoryNames[currentCategory]}
            </div>
            <h2 className="text-3xl font-bold mb-2">{currentLesson.title}</h2>
            <p className="text-pink-100 text-lg">{currentLesson.era}</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              onClick={handleNewLesson}
              className="flex-1 bg-pink-600 text-white px-8 py-4 rounded-2xl font-bold text-lg hover:bg-pink-700 hover:-translate-y-1 hover:shadow-xl transition-all flex items-center justify-center gap-3"
            >
              <span>🔄</span>
