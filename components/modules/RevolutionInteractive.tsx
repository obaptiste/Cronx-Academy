...
export default function RevolutionInteractive() {
  ...
  const generateRandomLesson = (completed: string[]) => {
    const allLessons = getAllRevolutionLessons();
  ...
  const markComplete = () => {
    if (currentLesson && typeof window !== 'undefined') {
      ...
      localStorage.setItem('completedRevolutionLessons', JSON.stringify(newCompleted));
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-8">
      <Link
        ...
        className="inline-flex items-center gap-2 bg-white text-blue-600 border-2 border-blue-600 px-5 py-3 rounded-xl font-semibold hover:bg-blue-600 hover:text-white transition-all hover:-translate-y-1 mb-6"
      >
        ← Back to History Quest
      </Link>

      {/* Header Section */}
      <div className="bg-white p-8 rounded-3xl shadow-xl mb-6 border-[3px] border-blue-500">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-gradient-to-br from-indigo-100 to-indigo-200 p-5 rounded-2xl">
            ...
            <div className="text-xl font-semibold text-gray-800">
              {revolutionCategoryNames[currentCategory]}
            </div>
          </div>
        </div>
      ...
      {viewMode === 'browse' && (
        <div className="space-y-6">
          {(Object.keys(revolutionTopics) as RevolutionTopicCategory[]).map((category) => (
            <div key={category} className="bg-white p-6 rounded-3xl shadow-lg">
              <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-3">
                <span className="text-3xl">{revolutionCategoryIcons[category]}</span>
                {revolutionCategoryNames[category]}
              </h2>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {revolutionTopics[category].map((lesson, idx) => {
      ...
      {viewMode === 'lesson' && currentLesson && (
        <div className="space-y-6">
          {/* Lesson Header */}
          <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-8 rounded-3xl shadow-xl">
            <div className="flex items-center gap-2 text-blue-200 mb-2">
              {revolutionCategoryIcons[currentCategory]} {revolutionCategoryNames[currentCategory]}
            </div>
